...
const KanbanBoard = () => {
  ...
  useEffect(() => {
    ...
    connectSocket.on("on-columns-initial", (cols) =>
      setColumns(cols.map(normalizeColumn))
    );
    connectSocket.on("on-column-created", (col) =>
      setColumns((prev) => [...prev, normalizeColumn(col)])
    );
    connectSocket.on("on-column-updated", (col) =>
      setColumns((prev) =>
        prev.map((c) => (c._id === col._id ? normalizeColumn(col) : c))
      )
    );
    connectSocket.on("on-column-deleted", (col) =>
      setColumns((prev) => prev.filter((c) => c._id !== col._id))
    );
    connectSocket.on("on-columns-reordered", (cols) =>
      setColumns(cols.map(normalizeColumn))
    );
    ...
    connectSocket.on(
      "on-task-order-changed",
      ({ columnId, taskId, newOrder }) => {
        setTasks((prev) => {
